...
| 拾うもの | mainの現状 | 備考 |
|---|---|---|
| ESLint導入 | 設定ファイルなし | 小さく単独で入れられる |
| nginx のセキュリティヘッダ／レート制限 | `deploy/nginx/` に2ファイルあるが未適用 | 本番設定なので反映は承認後 |
| 日次のrun-lock（LINE二重送信の防止） | 見当たらない | 誤送信に直結するので価値が高い |
| forbidden_actions をLINE push直前で実行 | `src/safety/forbidden_actions.ts` とテストはあるが、**本番の呼び出し箇所が無い** | ルールが書いてあるだけで効いていない状態 |

**次の一手**: この4点を1件ずつ小さなPRとして起票し直し、#34 は閉じる。
**優先**: run-lock と forbidden_actions の実行が先。どちらも「送ってはいけないものを送る」事故を止める。
...
1. #75 / #74 / #73 のレビューとマージ（衝突なし。すぐ進められる）
2. #34 から run-lock と forbidden_actions の実行を取り出して起票（誤送信を止める）
3. #17 の rebase 検証
4. #46 の ds-bundle だけの切り出し
5. #2 から弥生会計エクスポートの起票
6. #34 / #25 / #18 / #2 を閉じる（オーナー操作）
