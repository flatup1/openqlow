...
| `src/line_bot/webhook.ts` | 承認者と会員で経路を分岐（JIN承認 2026-08-09） | 承認者の経路は完全に従来どおり |
| `src/line_bot/withdrawal_intake.ts` | **新規**。会員メッセージ専用ハンドラ | — |
...
## 7. LINEフロー（配線済み・2026-08-09 JIN承認）

### webhook のルーティング

既存 webhook は承認者以外を丸ごと無視していた。会員の退会相談を受けるため、
**承認者と会員で経路を完全に分ける**形に変更した。

```
LINE webhook（署名検証）
  ├─ 承認者（JIN / バックアップ承認者）
  │    → 従来どおり: メディア取込 / 問い合わせ取込 / 承認・pushコマンド（変更なし）
  └─ 会員（承認者以外）
       → executeLineWithdrawalIntake のみ
         ・executeApprovalText へは絶対に渡さない（承認・pushコマンドを踏ませない）
         ・メディアも受け付けない
         ・返信トークンも取らない（内部の処理結果を会員へ返さない）
         ・退会の話でなければ何もしない
```

`JIN_LINE_USER_ID` 未設定の環境では、従来どおり全員を承認者として扱う（既存挙動を変えない）。

### 自動返信の二重ロック

会員へ実際に届くのは、次の**両方**が揃ったときだけ。片方でも欠けたら送らない。

| 条件 | 既定 |
|---|---|
| `OPENQLOW_WITHDRAWAL_AUTO_REPLY=true` | `false`（送らない） |
| `OPENQLOW_DRY_RUN=false` | `true`（送らない） |

送らない場合も相談は台帳に記録し、オーナーへ通知して下書きの場所を伝える（握りつぶさない）。

そのうえで、送る場合も次を守る。

- 会員へ送るのは**固定テンプレートのみ**。AIが生成した文章は送らない
- **実際に届いたときだけ**「案内済み」にする（dry-run や失敗を送信済みにしない）
- 特殊ケース（返金・弁護士・入院など）を検知したら**自動返信を止めて**人へ上げる
- オーナー通知に LINE userId や会員の発言そのものを載せない

### スタッフからのワンクリック送信

```
npm run crm -- withdrawal guide  <番号> --push   手続き案内を会員のLINEへ送信
npm run crm -- withdrawal notice <番号> --push   正式受付のご案内を会員のLINEへ送信
```

人が押した操作なので送信する。ただしここでも、**届いたときだけ**送信済みとして記録する。
