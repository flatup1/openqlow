...
画像は起動リンクでは渡せない（媒体・ブラウザの仕様）。各アイテムに `mediaHint` を載せ、
「投稿画面で写真を1枚手で付けてね」を案内する（instagram は画像必須なので強めの文言）。
画像まで自動添付したい場合は X公式API が必要で、これは Codex領域（別タスク）。

文字数制限: `CHAR_LIMITS` で媒体ごとの上限を持つ（**X=140字**、他は上限なし）。`charCount` は
絵文字・サロゲートペアを1字として数える。上限を超えると `withinLimit=false` となり、
**安全でも起動リンクは作らない**（人が縮めてから）。整形出力に「N字オーバー」を明示する。
