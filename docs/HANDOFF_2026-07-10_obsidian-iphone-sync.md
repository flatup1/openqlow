...
### STEP 0. 前提確認（5分）→ 確認済み（2026-07-10）

- vault用のGitHubリポジトリ確認結果: **`flatup1/flatup` はアクセス可能なリポジトリ一覧に存在しない**（未作成、またはClaude GitHub Appに権限がない）。
  - → `flatup1/flatup-vault` を **private** で新規作成する（オーナー承認後）。もし `flatup1/flatup` が実在するなら、privateであることを確認してそれを使う。
- 注意: **`flatup1/openqlow` はpublicリポジトリ**。メモ・日報・顧客に関する内容は絶対にopenqlowに入れない。vaultは必ずprivateにする。
- VPSの `/opt/obsidian-vault` が既にgitリポジトリかどうか確認する（`git -C /opt/obsidian-vault status`）。

### STEP 1. VPSのvaultをGitHubにつなぐ（30分・コード変更なし）

> ⚠️ 落とし穴が2つある。webhookサービスは `ProtectHome=true` で動くため **`~/.ssh` の鍵は読めない**。また初回pushで upstream を設定しないと `/push` の `pull --rebase` が失敗する。以下の手順はその両方を回避する。

1. **Deploy Key（書き込み可）を作る**。鍵はVPS上で生成し、秘密鍵はVPSから出さない。置き場所はホームではなく `/etc/openqlow/ssh/`:
   ```bash
   sudo mkdir -p /etc/openqlow/ssh
   sudo ssh-keygen -t ed25519 -N "" -C "openqlow-vault" -f /etc/openqlow/ssh/vault_deploy_key
   sudo chown -R openqlow:openqlow /etc/openqlow/ssh
   sudo chmod 700 /etc/openqlow/ssh && sudo chmod 600 /etc/openqlow/ssh/vault_deploy_key
   ```
   公開鍵（`.pub`）をGitHubのvaultリポジトリ → Settings → Deploy keys に「Allow write access」付きで登録。
2. `/etc/openqlow/openqlow.env` に追記（サービスがこの鍵でgitを使えるようにする）:
   ```bash
   GIT_SSH_COMMAND=ssh -i /etc/openqlow/ssh/vault_deploy_key -o IdentitiesOnly=yes -o StrictHostKeyChecking=accept-new
   ```
3. `/opt/obsidian-vault` で:
   - まだgit管理でなければ `git init -b main` → `git remote add origin git@github.com:flatup1/flatup-vault.git` → 初回コミット → **`git push -u origin main`**（`-u` 必須。upstreamが無いと `/push` が失敗する。初回pushはオーナー承認後）
   - すでにgit管理なら remote と upstream（`git rev-parse @{u}` が通るか）を確認
4. git設定は **リポジトリローカル**に入れる（`--global` はホーム配下なのでサービスから読めない）:
   ```bash
   git -C /opt/obsidian-vault config user.name "openqlow-bot"
   git -C /opt/obsidian-vault config user.email "openqlow-bot@users.noreply.github.com"
   ```
5. `.gitignore` に `.obsidian/workspace*` `.trash/` `.DS_Store` を入れる（端末ごとの画面状態を同期しない）。
6. systemdの `ReadWritePaths=/opt/obsidian-vault` は設定済みなので変更不要。最後にサービスと同じ条件で確認:
   ```bash
   sudo -u openqlow env GIT_SSH_COMMAND="ssh -i /etc/openqlow/ssh/vault_deploy_key -o IdentitiesOnly=yes" \
     git -C /opt/obsidian-vault push --dry-run
   ```
   これが通ればLINEの `/push` も通る。

### STEP 2. `/push` に pull を足す（コード変更・小）→ 実装済み（2026-07-10, このPR）

対象: `src/line_bot/commands.ts` の `pushVault()`

- 実装内容: ローカル変更があれば先に commit → **push の前に必ず `git pull --rebase`** → ahead 判定 → push。
- pull が衝突などで失敗したら `git rebase --abort` で元に戻し、LINEに「同期が衝突しました。手動確認が必要です。」と返す（勝手にmergeしない）。
- テスト: `src/line_bot/commands.test.ts` に pull→push の呼び出し順の検証と、pull失敗時のabort+エラーメッセージのテストを追加済み。`npm test` 緑。
