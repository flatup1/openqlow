...
- 保留は7:00以降の最初の実行でまとめて1回だけ届く。**届いた分だけ**保留から外す
  （送っている間に積まれた新しい分まで消さない）。
...
- 保留リストそのものが書けなかったときのために、手がかりをディスク側にも持つ。
  保存されているのに通知済みの印が無い下書きは、猶予（10分）を過ぎたら拾い直して通知する。
  範囲は当日と前日、1回50件まで。拾い直したあとは印が付くので、二度は届かない。
