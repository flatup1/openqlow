// 世界一優しい回答システム — 応答品質スコアラー。
//
// 実運用ログ（AIKA 受付 / OPENQLOW 投稿）で観測した失敗を二度と起こさないため、
// 顧客向け返信を出す前に 4 観点で自動採点する。各 25 点・合計 100 点。
//
//   ① empathy        悩みに寄り添えているか（機械的な質問攻めをしない）
//   ② naturalness    不自然な点はないか（スタック・既知事項の再質問をしない）
//   ③ nonDuplication 重複していないか（直近返信・文内反復の検出）
//   ④ kindness       世界一優しい回答か（既存 calculateKindnessScore + 安全チェック）
//
// 規範は knowledge/wiki/kindest-ai-response-policy.md と一致させる。

import { calculateKindnessScore, checkDraftSafety } from "./check.js";
