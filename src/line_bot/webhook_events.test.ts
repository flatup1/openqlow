...
// 同じバッチに入っていた承認者の指示まで消えていた。
{
  ...
  assert.equal(extracted.events.length, 1, "承認者のイベントは残る");
  assert.equal(extracted.events[0].text, "承認");
  assert.equal(extracted.events[0].userId, JIN);
...
// 非承認者だけのバッチは、これまでどおり ignored で返す（応答の形を変えない）。
{
  const extracted = extractLineEvents(batch(textEvent("U_guest", "こんにちは", "tok_guest")), APPROVERS);
  assert.deepEqual(extracted, { events: [], linePayload: true, ignored: "non_approver_user" });
}
