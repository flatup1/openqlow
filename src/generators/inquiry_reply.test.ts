...
// --- 退会・違約金・休会：体験誘導に化けさせず、canon案内＋人間確認へ（#5相当の回帰防止） ---
const penalty = generateInquiryReply({ message: "入会して半年ですが違約金はありますか？" });
assert(penalty.classification.temperature !== "high", `違約金 must not be high temperature, got ${penalty.classification.temperature}`);
assert(penalty.classification.nextAction !== "propose_schedule", "違約金 must not auto-propose 体験 schedule");
assert(penalty.replies.membershipConsult, "membership consult reply generated for 違約金 question");
assert(penalty.replies.polite.includes("退会届"), "penalty/cancellation reply includes 退会届 from canon");
assert(penalty.replies.polite.includes("契約内容"), "penalty reply defers 判定 to contract/staff (no 断定)");
assert(!penalty.replies.polite.includes("初回体験500円"), "penalty reply is NOT a trial invite");
assert(!penalty.replies.bookingFocused.includes("①お名前"), "違約金 does NOT get the trial booking form");
assert(penalty.replies.polite.trimEnd().endsWith("AIKA"), "membership reply is signed AIKA");

const quit = generateInquiryReply({ message: "退会したいです" });
assert(quit.classification.purpose === "退会・違約金のご相談", `退会 purpose, got ${quit.classification.purpose}`);
assert(quit.replies.membershipConsult, "membership consult for 退会");
assert(quit.replies.polite.includes("翌月末"), "退会 reply mentions 翌月末退会");
assert(quit.notes.some(n => n.includes("断定")), "notes warn 退会/休会/違約金 need staff confirmation");

const pause = generateInquiryReply({ message: "しばらく休会したいのですが" });
assert(pause.classification.temperature !== "high", "休会 not high temperature");
assert(pause.replies.polite.includes("休会"), "休会 reply mentions 休会");
assert(!pause.replies.polite.includes("初回体験500円"), "休会 reply is NOT a trial invite");
