...
mkdir -p "${ENV_DIR}" "${OPENQLOW_ROOT}/logs" "${OPENQLOW_ROOT}/drafts" "${OPENQLOW_ROOT}/state" "${OPENQLOW_ROOT}/ad-line-data" "${OBSIDIAN_VAULT_ROOT}"
...
echo "  Note: openqlow-morning.timer fires at 06:00 JST daily — set OPENQLOW_MORNING_PUSH_DISABLED=true to disable."
echo "  Note: 広告LINEは承認後にだけ 'systemctl enable --now openqlow-ad-line.service' を実行する。"
echo "  Note: 広告日報のドライランは承認後にだけ 'systemctl enable --now openqlow-ad-daily-report.timer' を実行する。LINE送信は行わない。"
