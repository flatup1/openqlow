...
結論: **MacにBunを入れれば、3コマンドで自分専用の動画編集ソフトが立ち上がる。** 動画はネットに出ず、Macの中だけで扱える。Docker は無くてよい。
...
| 物 | 要否 | 用途 | 入れ方 |
|---|---|---|---|
| Bun | **必須** | 画面を動かす | `curl -fsSL https://bun.sh/install \| bash` |
| git | **必須** | 本体の取得 | `xcode-select --install` |
| Docker Desktop | 任意 | ログイン機能などサーバーが要る部分 | https://docs.docker.com/desktop/ |

**Docker は編集と書き出しには要らない。** データベースもRedisも全部止めた状態で、取り込み→タイムライン配置→書き出しまで成功することを確認済み。Docker が無ければ `setup.sh` が自動で飛ばして先に進む。
...
- **PostgreSQL・Redis・Upstash互換サーバーを全部止めた状態でも、上の一連（作成→取り込み→配置→書き出し）がすべて成功した**。プロジェクトと素材はブラウザ内に保存されるため、編集にサーバーは要らない

## 7. 未確認のこと（正直に書く）

- **Dockerでの起動は未確認**。検証したコンテナではDockerイメージの取得が組織のネットワーク方針で拒否された。ただし Docker は任意扱いにしたので、編集と書き出しだけなら影響しない。ログイン機能を使いたくなった時点で確認すればよい。
